...
if (!ensureLocalFile(
    strpos($filename, 'processed_') === 0 ? 'processed' : 'presentations',
    $filename,
    $filepath
)) {
    die("File not found.");
...
if (!MOCK_MODE) {
    if (OPENAI_API_KEY === '') {
        die('OPENAI_API_KEY is required when mock mode is disabled.');
    }
...
<body>
    <div class="container process-page">
        ...
        <div id='progress'>
                ...
                if ($slideAnalysis['type'] === 'text_only' && !empty($slideAnalysis['text'])) {
                    ...
                    if ($extractedImagePath && file_exists($extractedImagePath)) {
                        ...
                        if ($description) {
                            ...
                            persistFile('ai_texts', basename($textPath), $textPath);
            ...
            $newZip->close();
            persistFile('processed', basename($processedFilename), $processedFilename);
...
function generateQRCode($url, $filename)
{
    ...
    persistFile('qrcodes', basename($qrPath), $qrPath);
